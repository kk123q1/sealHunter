//------------------------------------------------------------------------------//
//
// MODULE   : AIRespawnPoint.cpp
//
// PURPOSE  : AIRespawnPoint - Implementation
//
// CREATED  : 08/27/2002
//
...
ADD_REALPROP(AIRespawnPointValue, 0.0f)
END_CLASS_DEFAULT_FLAGS(AIRespawnPoint, BaseClass, LTNULL, LTNULL, CF_ALWAYSLOAD)
...
//-----------------------------------------------------------------------------
AIRespawnPoint::AIRespawnPoint() :
	m_nAIRespawnPointType(0),
	m_fAIRespawnPointValue(0.0f),
	m_iHitPoints(g_MaxAIRespawnPointLife),
...
//-----------------------------------------------------------------------------
//	AIRespawnPoint::EngineMessageFn(uint32 messageID, void *pData, float fData)
//
//-----------------------------------------------------------------------------
uint32 AIRespawnPoint::EngineMessageFn(uint32 messageID, void* pData, float fData)
{
	...
	case MID_MODELSTRINGKEY:
	{
		...
		for (int i = 0; i < pArgList->argc; i++)
		{
			...
			{
				PlayClientFX("AIRespawnPointYelp", m_hObject, LTNULL, LTNULL, 0);
			}
...
//-----------------------------------------------------------------------------
//	AIRespawnPoint::ObjectMessageFn(HOBJECT hSender, void *pData, float fData)
//
//-----------------------------------------------------------------------------
uint32 AIRespawnPoint::ObjectMessageFn(HOBJECT hSender, ILTMessage_Read* pMsg)
{
	pMsg->SeekTo(0);
	...
	case OBJ_MID_DAMAGE:
	{
		// Stop moving if the AIRespawnPoint is hit.
		StopMoving();
		...
		if (m_iHitPoints < 1)
		{
			...
			g_pLTServer->CPrint("AIRespawnPoint Death Rand: %f", fRand);
			...
			PlayClientFX("AIRespawnPointFall", m_hObject, LTNULL, LTNULL, 0);
		}
		else
		{
...
//-----------------------------------------------------------------------------
//	AIRespawnPoint::PreCreate(void *pData, float fData)
//
//-----------------------------------------------------------------------------
uint32 AIRespawnPoint::PreCreate(void* pData, float fData)
{
	// Let parent class handle it first
	BaseClass::EngineMessageFn(MID_PRECREATE, pData, fData);
	//g_pLTServer->CPrint("here now 11111111111111111111111111111111111111111111");
	...
	// Randomize the AIRespawnPoint's initial rotation.
...
//-----------------------------------------------------------------------------
//	AIRespawnPoint::Update()
//
//-----------------------------------------------------------------------------
uint32 AIRespawnPoint::Update()
{
	g_pLTServer->CPrint("here now 11111111111111111111111111111111111111111111");
	switch (m_iAction)
	{
	...
	case ACTION_DEAD:
	{
		...
		{
			// Remove the AIRespawnPoint object.
			...
			if (iNumObjects > 0)
			{
				// Decrement the number of AIRespawnPoints.
	...
	default:
	{
		g_pLTServer->CPrint("(AIRespawnPoint) Unknown AIRespawnPoint action!");
		break;
	}
	}
...
//-----------------------------------------------------------------------------
//	AIRespawnPoint::TouchNotify(void *pData, float fData)
//
//-----------------------------------------------------------------------------
uint32 AIRespawnPoint::TouchNotify(void* pData, float fData)
{
...
//-----------------------------------------------------------------------------
//	AIRespawnPoint::ReadProps(ObjectCreateStruct* pStruct)
//
//-----------------------------------------------------------------------------
void AIRespawnPoint::ReadProps(ObjectCreateStruct* pStruct)
{
	...
	g_pLTServer->GetPropReal("AIRespawnPointValue", &m_fAIRespawnPointValue);
}



//-----------------------------------------------------------------------------
//	AIRespawnPoint::PlaySound()
//
//-----------------------------------------------------------------------------
void AIRespawnPoint::PlaySound()
{
	/*
		...
		SAFE_STRCPY(playSoundInfo.m_szSoundName, "Snd/AIRespawnPoint2.wav");

		LTRESULT hResult = g_pLTServer->SoundMgr()->PlaySound(&playSoundInfo, hSnd);

		if(hResult != LT_OK)
		{
			g_pLTServer->CPrint("(AIRespawnPoint) Error playing sound");
		}
	*/
}
...
//-----------------------------------------------------------------------------
//	AIRespawnPoint::ChangeAnimation()
//
//-----------------------------------------------------------------------------
void AIRespawnPoint::ChangeAnimation(const char* sAnimName, bool bLooping, bool bInterable)
{
...
//-----------------------------------------------------------------------------
//	AIRespawnPoint::Spawn()
//
//-----------------------------------------------------------------------------
void AIRespawnPoint::Spawn()
{
	LTVector Pos;
	LTRotation Rot;
	GetPosRot(&Pos, &Rot);
	...
	if (RayCastToGround(Pos, Rot, &IntersectPt))
	{
		...
		{
			g_pLTServer->CPrint("(AIRespawnPoint) AIRespawnPoint spawned below ground!");
		}

		// Offset the falling distance by the AIRespawnPoint's bounding box.
		LTVector vDims;
	...
	{
		g_pLTServer->CPrint("(AIRespawnPoint) Could not find ground!");
	}
}
...
//
//-----------------------------------------------------------------------------
void AIRespawnPoint::Warp()
{
	// Make the AIRespawnPoint fall from the sky.
...
//-----------------------------------------------------------------------------
//	AIRespawnPoint::StartMoving()
//
//		Set an initial velocity for the AIRespawnPoint.
//-----------------------------------------------------------------------------
void AIRespawnPoint::StartMoving(float Speed)
{
	LTVector vPos;
	LTRotation rRot;
	GetPosRot(&vPos, &rRot);
...
//-----------------------------------------------------------------------------
//	AIRespawnPoint::StopMoving()
//
//		Set the velocity to zero.
//-----------------------------------------------------------------------------
void AIRespawnPoint::StopMoving()
{
...
//-----------------------------------------------------------------------------
//	AIRespawnPoint::FlopAround()
//
//-----------------------------------------------------------------------------
void AIRespawnPoint::FlopAround(int& Counter, float Speed)
{
...
//-----------------------------------------------------------------------------
//	AIRespawnPoint::GetPosRot()
//
//		Helper function for getting the object position and rotation.
//-----------------------------------------------------------------------------
void AIRespawnPoint::GetPosRot(LTVector* pPos, LTRotation* pRot)
{
	// position
	if (pPos != NULL)
	{
		g_pLTServer->GetObjectPos(m_hObject, pPos);
		g_pLTServer->CPrint("AIRespawnPoint position:%.2f,%.2f,%.2f", pPos->x, pPos->y, pPos->z);
	}
	else
	{
		g_pLTServer->CPrint("nulllllllllllllllllllllllllllllll");
	}
...
//-----------------------------------------------------------------------------
//	AIRespawnPoint::RayCastToGround()
//
//		Casts a ray straight down from the object position and returns the
//		point of intersection.
//-----------------------------------------------------------------------------
bool AIRespawnPoint::RayCastToGround(const LTVector& Pos, const LTRotation& Rot, LTVector* pIntersectPt)
{
...
void AIRespawnPoint::SendKillStealChatMessage(HOBJECT hFirst, HOBJECT hKiller)
{
...
//
//-----------------------------------------------------------------------------
void AIRespawnPoint::Die()
{
	StopMoving();

	// Calc AIRespawnPoint kill value
	bool bKillSteal = ((m_hAssasin != m_hFirstAttacker) && (m_fFirstAttackerDmg > ((float)g_MaxAIRespawnPointLife) / 4.0f));
	float fAIRespawnPointValue = g_AIRespawnPointPeltValue * ((bKillSteal) ? 2.0f : 1.0f);

	// DIE!!!!
	if (bKillSteal)
	{
		PlayClientFX("AIRespawnPointDieKillSteal", m_hObject, LTNULL, LTNULL, 0);
		PlayClientFX("KillStealOwned", m_hFirstAttacker, LTNULL, LTNULL, 0);
		SendKillStealChatMessage(m_hFirstAttacker, m_hAssasin);
	}
	else
	{
		PlayClientFX("AIRespawnPointDie", m_hObject, LTNULL, LTNULL, 0);
	}
	...
	pMsg->Writefloat(fAIRespawnPointValue);
