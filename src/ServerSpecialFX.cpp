...
END_CLASS_DEFAULT_FLAGS_PLUGIN( SpecialFX, BaseClass, NULL, NULL, 0, SpecialFXPlugin )
...
{
	//g_pLTServer->CPrint("22222222222222222 !!!!!!!!!!!!!!!!!!!!!!");
	switch (messageID)
	{
		case MID_PRECREATE :
		{
...
//
// ----------------------------------------------------------------------- //
uint32  SpecialFX::ObjectMessageFn(HOBJECT hSender, ILTMessage_Read *pMsg)
{
    uint8 msg = pMsg->Readuint8();
    switch(msg)
    {
    case 0:
        break;
...
bool SpecialFX::OnTrigger(HOBJECT hSender, const CParsedMsg &cMsg)
{
	...
	{
		//return BaseClass::OnTrigger(hSender, cMsg);
	}
...
void SpecialFX::TurnOFF()
{
	...
	if (m_bLoop)
	{
		...
		g_pLTSCommon->SetObjectFlags( m_hObject, OFT_Flags, 0, FLAG_FORCECLIENTUPDATE );

		g_pLTSCommon->SetObjectFlags( m_hObject, OFT_User, 0, USRFLG_SFX_ON );

	}

	m_bIsOn = false;

    g_pLTServer->CPrint("TurnOFF!!!");
}
...
void SpecialFX::OnLoad(ILTMessage_Read *pMsg, uint32 dwFlags)
{
	...
	if (!m_bLoop)
	{
		g_pLTSCommon->SetObjectFlags(m_hObject, OFT_Flags, 0, FLAGMASK_ALL);
...
void SpecialFX::ReadProps(ObjectCreateStruct *pOcs)
{
	...
	{
		g_pLTServer->CPrint(GenProp.m_String);
