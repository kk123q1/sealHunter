...
LTRESULT CLTServerShell::OnServerInitialized()
{
    ...
    g_pLTSPhysics->SetStairHeight(36.0f);
...
void CLTServerShell::OnMessage(HCLIENT hSender, ILTMessage_Read *pMessage)
{
	...
	//g_pLTServer->CPrint("%d",messageID);
	switch(messageID)
	{
	...
	case MSG_CS_VELANDROT:
		{
			...
			//g_pLTServer->CPrint("%.2f,%.2f,%.2f", vPos.x, vPos.y, vPos.z);
            //g_pLTServer->MoveObject(hPlayer, &vPos);
            g_pLTServer->SetObjectRotation(hPlayer, &rRot);

			g_pLTServer->SetClientViewPos(hSender, &vPos);

			//g_pLTServer->CPrint("CASE10101010111111111111111111111");
		}
		break;
    case MSG_CS_ANIM:
        {
    ...
    case MSG_CS_CHAT:
        ...
        break;
	case MSG_C4_EXPLOSION: 
	{
		vPos = pMessage->ReadLTVector();
		//g_pLTServer->CPrint("%.2f,%.2f,%.2f", vPos.x, vPos.y, vPos.z);
		LTVector Pos(100.0f, -5700.0f, -104.14f);
		LTVector PosLight(100.0f, -5950.0f, -104.14f);
		LTVector PosLight1(3406.0f, -5950.0f, 1905.0f);
		//rRot = pMessage->ReadLTRotation();
		rRot.Identity();
		//PlayClientFX("c4-explode", LTNULL, &Pos, &rRot,0);
		//PlayClientFX("c4-red-1", LTNULL, &Pos, &rRot, 0); 
		//PlayClientFX("c4-Green", LTNULL, &Pos, &rRot, 0);
		PlayClientFX("Zforce_C4_light_Big", LTNULL, &PosLight, &rRot, FLAG_UPDATEUNSEEN | FLAG_CASTSHADOWS | FLAG_FULLPOSITIONRES| FLAG_ROTATABLESPRITE);
		PlayClientFX("Zforce_C4_light_Big", LTNULL, &PosLight1, &rRot, FLAG_UPDATEUNSEEN | FLAG_CASTSHADOWS | FLAG_FULLPOSITIONRES| FLAG_ROTATABLESPRITE);
...
{
	//AIRespawnPoint.OnPlayerPositionChanged(vPos);
